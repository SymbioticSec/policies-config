...
def get_trivy_download_links(version: str):
    ...
    links = {
        label: f"{BASE_TRIVY_RELEASE_URL}/v{version}/trivy_{version}_{system}.{ext}"
...
def generate_scanner_config(scanner_config_path: Path) -> dict:
    """Reads, processes and returns the scanner configuration."""

    base_scanner_config = read_content(scanner_config_path)
    dl_links = get_trivy_download_links(base_scanner_config["iac"]["scanner-version"])
    base_scanner_config["iac"].update({"scanner-dl-links": dl_links})
    return base_scanner_config


def get_rules_files(rules_path: Path) -> dict[str, dict]:
    """Returns all specific rules configs by ID."""

    files = rules_path.rglob("*-AVD-*.yml")
    return {file.stem: read_content(file) for file in files}


def partition_rules_disabled(
    rules: dict[str, dict]
...
def generate_rules_config(rules_config_path: Path) -> dict:
    """Reads, processes and returns the rules configuration from each rule file ."""

    defaults = read_content(rules_config_path / "iac" / "defaults.yml")
    rules_configs = get_rules_files(rules_config_path)
    enabled_rules, disabled_rules = partition_rules_disabled(rules_configs)
...
def generate_config(directory_path: Path):
    ...
    scanner_config = generate_scanner_config(directory_path / SCANNER_CONFIG_FILENAME)
    rules_config = generate_rules_config(directory_path / "rules-config")
